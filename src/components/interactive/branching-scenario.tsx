...
export function BranchingScenario({ title, scenario, choices, onComplete }: Props) {
  ...
  return (
    <div className="my-3 rounded-xl border bg-card p-4">
      {title && (
        <p className="mb-2 text-xs font-semibold text-ulul-albab-primary uppercase tracking-wider">{title}</p>
      )}
      ...
      {!selectedId ? (
        ...
        <div className="rounded-lg border-l-4 border-ulul-albab-primary bg-muted/20 px-4 py-3">
          <p className="text-xs font-medium text-ulul-albab-primary mb-1">Outcome</p>
